...
async function generateGrid(inputPath: string): Promise<string> {
  ...
  const fps = 1; // Reduced to 1 frame per second
  const totalFrames = duration * fps;

  // Calculate optimal grid dimensions
  const targetFrameSize = 600; // Desired frame size
  const aspectRatio = 16 / 9; // Common video aspect ratio

  // Calculate grid dimensions based on total frames
  // Try to keep grid as square as possible while fitting all frames
  const gridWidth = Math.ceil(Math.sqrt(totalFrames * aspectRatio));
  ...
  // Calculate optimal frame size to fit within reasonable output dimensions
  const maxOutputWidth = gridWidth * targetFrameSize;
  const frameWidth = Math.min(
    targetFrameSize,
    Math.floor(maxOutputWidth / gridWidth)
  );
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(inputPath)
      ...
      .videoFilter([
        ...
        `drawtext=text='%{pts\\:hms}':fontsize=${
          frameWidth / 12
        }:fontcolor=white:box=1:boxcolor=black@0.5:x=(w-text_w)/2:y=h-${
          frameHeight / 10
        }`,
        `tile=${gridWidth}x${gridHeight}:padding=1:margin=2:color=black`,
      ])
      .outputOptions([
        "-frames:v 1",
        "-update 1",
