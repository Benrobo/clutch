...
async function detectScenes(inputPath: string): Promise<SceneInfo[]> {
  ...
  const ThreshHold = 0.1; // Lower threshold to detect more scenes
      ...
      .on("stderr", (stderrLine) => {
        if (stderrLine.includes("[Parsed_showinfo_1")) {
          ...
          if (stderrLine.includes("timecode") && currentScene) {
            ...
            if (timecodeMatch) {
              currentScene.timecode = timecodeMatch[1];
              // Only save I-frames as scene changes
              if (currentScene.type === "I") {
                scenes.push(currentScene as SceneInfo);
              }
              currentScene = {};
...
async function extractScenes(
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    const endTime = nextScene
      ? Math.min(
          nextScene.timestamp,
          scene.timestamp + config.postBufferSeconds
        )
      : scene.timestamp + config.postBufferSeconds;
    ...
    const outputPath = path.join(
      processingDir,
      `scene${(i + 1).toString().padStart(3, "0")}.mp4`
    );
    ...
    await new Promise<void>((resolve, reject) => {
      ffmpeg(inputPath)
        ...
        .outputOptions([
          ...
          "-movflags",
          "+faststart",
          // First scale to match target height while maintaining aspect ratio
          // Then crop from center to get target width
          "-vf",
          `scale=-2:${OUTPUT_HEIGHT},crop=${OUTPUT_WIDTH}:${OUTPUT_HEIGHT}:(iw-${OUTPUT_WIDTH})/2:0`,
        ])
        .on("end", () => {
          console.log(`Extracted scene ${i + 1} to ${outputPath}`);
          outputPaths.push(outputPath);
          resolve();
        })
        .on("error", (err) => {
          console.error(`Error extracting scene ${i + 1}:`, err.message);
          reject(err);
        })
        .save(outputPath);
    });
  ...
  const metadata = scenes.map((scene, index) => ({
    sceneFile: `scene${(index + 1).toString().padStart(3, "0")}.mp4`,
    timestamp: scene.timestamp,
    timecode: scene.timecode,
    duration:
      scene.timestamp +
      config.postBufferSeconds -
      (scene.timestamp - config.preBufferSeconds),
  }));
...
function mergeCloseScenes(
  ...
  // Sort scenes by timestamp to ensure proper ordering
  const sortedScenes = [...scenes].sort((a, b) => a.timestamp - b.timestamp);
  const mergedScenes: SceneInfo[] = [];

  let i = 0;
  while (i < sortedScenes.length) {
    const currentScene = sortedScenes[i];
    const nearbyScenes = [currentScene];
    let j = i + 1;

    // Find all scenes within minSceneGap of current scene
    while (
      j < sortedScenes.length &&
      sortedScenes[j].timestamp - currentScene.timestamp < minSceneGap
    ) {
      nearbyScenes.push(sortedScenes[j]);
      j++;
    }

    if (nearbyScenes.length === 1) {
      // If no nearby scenes, just add the current scene
      mergedScenes.push(currentScene);
    } else {
      // Sort nearby scenes by their importance score
      const scoredScenes = nearbyScenes
        .map((scene) => ({
          scene,
          score: calculateSceneScore(scene),
        }))
        .sort((a, b) => b.score - a.score);

      // Keep the top 2 scenes if they're significantly different
      if (
        scoredScenes.length >= 2 &&
        scoredScenes[0].score > scoredScenes[1].score * 1.5
      ) {
        mergedScenes.push(scoredScenes[0].scene);
      } else {
        // Just keep the highest scored scene
        mergedScenes.push(scoredScenes[0].scene);
      }
    }

    i = j; // Skip to next group of scenes
  }

  return mergedScenes.sort((a, b) => a.timestamp - b.timestamp);
}
...
async function processVideo(inputPath: string, config: SceneExtractionConfig) {
  try {
    ...
    const minSceneGap = Math.max(
      3,
      config.preBufferSeconds + config.postBufferSeconds
    ); // At least 3 seconds gap
