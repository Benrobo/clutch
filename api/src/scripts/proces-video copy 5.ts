...
async function detectScenes(inputPath: string): Promise<SceneInfo[]> {
  ...
  const ThreshHold = 0.3; // Lower threshold to detect more scenes
      ...
      .on("stderr", (stderrLine) => {
        if (stderrLine.includes("[Parsed_showinfo_1")) {
          ...
          if (stderrLine.includes("n:")) {
            const frameMatch = stderrLine.match(/n:\s+(\d+)/); // Frame number
            const ptsMatch = stderrLine.match(/pts_time:(\d+\.\d+)/); // Timestamp
            const typeMatch = stderrLine.match(/type:([IP])/); // Frame type
...
async function extractScenes(
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    const endTime = nextScene
      ? Math.min(
          ...
          scene.timestamp + config.postBufferSeconds + 10 // Ensure consistent 10-second clips
        )
      : scene.timestamp + config.postBufferSeconds + 10; // Last clip also 10 seconds
  ...
  const metadata = scenes.map((scene, index) => ({
    ...
    duration: 10, // All clips are now 10 seconds
  }));
...
function mergeCloseScenes(
  ...
  for (let i = 1; i < scenes.length; i++) {
    ...
    // If gap is too small, keep the more significant scene (higher stddev)
    if (nextScene.timestamp - currentScene.timestamp < minSceneGap) {
      const currentStddev = Math.max(...currentScene.stddev);
      const nextStddev = Math.max(...nextScene.stddev);

      if (nextStddev > currentStddev) {
        currentScene = nextScene; // Keep the scene with more visual change
      }
    } else {
...
async function processVideo(inputPath: string, config: SceneExtractionConfig) {
  try {
    ...
    const minSceneGap = config.preBufferSeconds + config.postBufferSeconds + 2;
