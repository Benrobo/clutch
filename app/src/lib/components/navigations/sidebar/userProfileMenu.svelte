<script lang="ts">
  ...
  import { getCreditBalance } from "@/http/requests";
  ...
  $: creditBalanceQuery = createQuery({
  ...
  $: creditBalance = $creditBalanceQuery.data?.data?.balance ?? 0;
...
<Flex className="w-full h-auto flex-col gap-3 p-2">
  ...
  {#if $authStore?.user?.subscription_plan !== "PRO"}
    <button
      ...
      on:click={() => {
        ...
        setTimeout(() => {
          globalStore.toggleUpgradeModal(true);
        }, 100);
      }}
    >
  ...
  <button
    ...
    on:click={() => {
      ...
      if (lowerCase(plan) === "free") {
        ...
        if ($isMobile) globalStore.toggleSidebar(false);
        globalStore.toggleCreditTopUpModal(true);
      }
    }}
  >
    <Flex
      className={cn(
        ...
        !$creditBalanceQuery.isFetching &&
          $creditBalanceQuery.data?.data?.balance <= 1
          ? "animate-pulse border-red-200 border-[2px]"
          : "animate-none"
      )}
    >
      <Flex className="w-full flex-row items-center justify-start">
        <CircleDollarSign class="stroke-white-300" size={20} />
        <h1 class="text-white-100 font-poppins font-medium text-sm">
          <!-- {#if $creditBalanceQuery.isFetching}
      ...
      <p
        class="text-white-200 font-normal text-[10px] text-nowrap translate-y-1"
      >
        Credits Left
      </p>
    </Flex>
  ...
  <Flex
    className="w-full h-auto flex-col gap-1 border-[1px] border-gray-101/50 p-2 rounded-md"
  >
    <button
